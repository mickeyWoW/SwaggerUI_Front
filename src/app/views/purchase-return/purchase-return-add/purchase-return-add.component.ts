...
export class PurchaseReturnAddComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.isLoader = true;
    this.purchaseReturnService.createPurchaseReturn(this.form.value).subscribe(
      (response) => {
        console.log("purchase", response);
        Swal.fire("Purchase Return Created", "", "success");
        this.isLoader = false;
        this.grandTotal = 0;
        this.subTotal = 0;
        this.orderTax = 0;
        this.router.navigate(["/purchase-return/view"]);
        this.form.reset();
      },
      (error) => {
        this.isLoader = false;
