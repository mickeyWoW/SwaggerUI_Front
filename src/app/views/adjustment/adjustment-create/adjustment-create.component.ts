...
@Component({
  ...
  styleUrls: ["./adjustment-create.component.scss"],
})
export class AdjustmentCreateComponent implements OnInit {
  ...
  onTypeChange(e, i) {
    console.log(e);
    this.searchedProducts[i].dummyType = e.id;
  }
  ...
  onSubmit() {
    ...
    this.isLoader = true;
    this.adjustmentService.createAdjustment(this.form.value).subscribe(
      (response) => {
        console.log("adjustments", response);
        Swal.fire("Adjustment Created", "", "success");
        this.isLoader = false;
        this.form.reset();
        this.products = null;
        this.search = null;
